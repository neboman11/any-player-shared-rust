...
// Force the OS identifier to "linux" so librespot always presents as a Linux/
// desktop client. This is intentional: on Android the three Spotify endpoints
// behave differently for the "android" OS identifier:
//   • clienttoken.spotify.com only grants tokens for ANDROID_CLIENT_ID
//   • login5.spotify.com rejects StoredCredentials issued for ANDROID_CLIENT_ID
//     with INVALID_CREDENTIALS (they are signed for a third-party OAuth client
//     that is not acceptable to Login5).
// By pretending to be Linux (desktop), librespot uses KEYMASTER_CLIENT_ID for
// both the client-token request and the Login5 StoredCredential exchange, which
// is the same flow used by the official Spotify desktop client and works
// correctly from any network, including Android emulator and physical devices.
...
pub fn os_version() -> String {
    // sysinfo does not support Android; return a fixed version string that
    // Spotify's client-token and Login5 endpoints accept (API level 30 / Android 11).
    // Without a valid version, Spotify returns INVALID_CREDENTIALS.
