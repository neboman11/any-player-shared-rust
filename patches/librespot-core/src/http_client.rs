...
impl HttpClient {
    ...
    pub fn request_fut(&self, mut req: Request<Bytes>) -> Result<ResponseFuture, Error> {
        ...
        // Preserve a caller-supplied User-Agent (e.g. Login5 uses the Linux
        // desktop UA with KEYMASTER_CLIENT_ID to match how the desktop works).
        headers_mut.entry(USER_AGENT).or_insert_with(|| self.user_agent.clone());
