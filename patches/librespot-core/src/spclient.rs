...
impl SpClient {
    ...
    pub async fn client_token(&self) -> Result<String, Error> {
        ...
        // Use the session's client_id for the client-token request. With OS forced to
        // "linux" in config.rs, this is always KEYMASTER_CLIENT_ID, which is accepted
        // by clienttoken.spotify.com for the Linux/desktop platform.
        ...
        // OS is "linux" (forced in config.rs) so this always builds Linux platform data.
        let os = OS;
